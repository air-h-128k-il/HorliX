...
#include "/usr/local/include/openjpeg-2.3/opj_config.h"
...
- (BOOL) HOROS_showsFullScreenButton {
    NSLog(@"FullScreenButton etc.... doubtful by air");
    return NO;
}
@end
...
+ (BOOL)hasMacOSXHighSierra//add definition by air
{
    NSOperatingSystemVersion v = [self.class operatingSystemVersion];
...
{	
	...
	for(NSString *path in pluginsArray)
	{
        ...
        NSLog(@"installPlugins in AppController.m line 1029");
...
{
    if( sender)
        [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:URL_HOROS_VIEWER@"/HorliX-64bit.html"]];
    else
    {
        NSArray* urls = [NSArray arrayWithObject: [NSURL URLWithString:URL_HOROS_VIEWER@"/HorliX-64bit.html"]];
...
//				Altivec = HasAltiVec();
                ...
                NSLog(@"HorliX version: %@ - %@ - %@", [[[NSBundle mainBundle] infoDictionary] objectForKey:(NSString *)kCFBundleVersionKey], [[[NSBundle mainBundle] infoDictionary] objectForKey: @"CFBundleShortVersionString"], bits);
...
//				[[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"DONTCOPYWLWWSETTINGS"];
				...
				[[NSUserDefaults standardUserDefaults] setObject: NSLocalizedString( @"(~/Library/Application Support/HorliX App/)", nil) forKey:@"DefaultDatabasePath"];
				#else
...
#endif
    // vanis FRFeedbackReporter by air
...
- (IBAction) checkForUpdates: (id) sender
{
    ...
    url = [NSURL URLWithString:URL_HOROS_VERSION];// here version check! by air
	...
	if( url)
	{
		...
		NSString *latestVersionNumber = [productVersionDict valueForKey:@"HorliX"];
...
-(void)_receivingIconUpdate {
	if (!_receivingDict.count)
		[NSApp setApplicationIconImage:[NSImage imageNamed:@"HorliX.icns"]];
