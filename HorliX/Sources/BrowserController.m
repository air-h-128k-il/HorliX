...
-(IBAction) createDatabaseFolder:(id) sender
{
    ...
    [oPanel beginWithCompletionHandler:^(NSInteger result) {
        ...
        if( [[location lastPathComponent] isEqualToString:@"HorliX Data"])
            location = [location stringByDeletingLastPathComponent];
        
        if( [[location lastPathComponent] isEqualToString:@"DATABASE.noindex"] && [[[location stringByDeletingLastPathComponent] lastPathComponent] isEqualToString:@"HorliX Data"])
...
- (NSString*)getDatabaseIndexFileFor: (NSString*)path {  // __deprecated
    ...
    if( [[NSFileManager defaultManager] fileExistsAtPath: path isDirectory: &isDirectory])
    {
        ...
        {
            // Default SQL file
            NSString	*index = [[path stringByAppendingPathComponent:@"HorliX Data"] stringByAppendingPathComponent:@"Database.sql"];
        ...
        switch (COPYDATABASEMODE)
        {
            ...
            case ask:
                switch (NSRunInformationalAlertPanel(
                                                     NSLocalizedString(@"HorliX Database", nil),
                                                     NSLocalizedString(@"Should I copy these files in HorliX Database folder, or only copy links to these files?", nil),
...
- (IBAction) saveDBListAs:(id) sender
{
    ...
    sPanel.nameFieldStringValue = NSLocalizedString(@"HorliX Database List", nil);
...
#pragma clang diagnostic pop
        ...
        @catch( NSException *ne)
        {
            ...
            NSString *message = [NSString stringWithFormat: NSLocalizedString(@"A problem occured during start-up of HorliX:\r\r%@\r\r%@",nil), [ne description], [AppController printStackTrace: ne]];
...
- (void)openDatabasePath: (NSString*)path
{
    ...
    {
        N2LogExceptionWithStackTrace(e);
        NSRunAlertPanel(NSLocalizedString(@"HorliX Database", nil), NSLocalizedString( @"HorliX cannot read/create this file/folder. Permissions error?", nil), nil, nil, nil);
        [self resetToLocalDatabase];
    }
