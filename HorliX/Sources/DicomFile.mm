    ...
    if( [extension isEqualToString:@"hdr"])
    {
        if ([[NSFileManager defaultManager] fileExistsAtPath:[[filePath stringByDeletingPathExtension] stringByAppendingPathExtension:@"img"]] == YES)
        {
            ...
            if( [file length] == 348)
            {
                ...
                date = [[HorliX dateWithString:[NSString stringWithCString: Analyze->hist.exp_date encoding: NSISOLatin1StringEncoding] calendarFormat:@"%Y%m%d"] retain];//modify Horos to HorliX by air
...
+ (NSString*) patientUID: (id) src
{
    ...
    if( gUsePatientBirthDateForUID)
        patientBirthDate = [HorliX:[NSDate dateWithTimeIntervalSinceReferenceDate:[[src valueForKey:@"patientBirthDate"] timeIntervalSinceReferenceDate]] descriptionWithCalendarFormat:@"%Y%m%d"];//Horos to HorliX
