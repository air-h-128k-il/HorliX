...
+ (void) loadHorliXPluginAtPath:(NSString*) path
{
    [self loadPluginBundle:path];
}
...
+ (void) loadPluginAtPath: (NSString*) path
{
    ...
    else if ( [[name pathExtension] isEqualToString:@"horlixplugin"] )
    {
        //[PluginManager loadHorliXPluginAtPath:path];
        [PluginManager loadHorosPluginAtPath:path];
    }
...
+ (void) discoverPlugins
{
	@try
	{
		NSString	*appSupport = @"Library/Application Support/HorliX/";//modify-by-air
        NSString	*appAppStoreSupport = @"Library/Application Support/HorliX App/";//modify-by-air
...
{
    #ifdef MACAPPSTORE
	return @"Library/Application Support/HorliX App/Plugins/";// modify-by-air
    #else
    return @"Library/Application Support/HorliX/Plugins/";//modify-by-air
    #endif
}
...
{
    #ifdef MACAPPSTORE
	return @"Library/Application Support/HorliX App/Plugins Disabled/";//modify-by-air
    #else
    return @"Library/Application Support/HorliX/Plugins Disabled/";//modify-by-air
    #endif
}
...
//		BOOL allUsers = ([path isEqualToString:sysActivePath] || [path isEqualToString:sysInactivePath]);
        ...
        while(name = [e nextObject])
		{
			if( [[name pathExtension] isEqualToString:@"horlixplugin"] ||  [[name pathExtension] isEqualToString:@"horosplugin"] || [[name pathExtension] isEqualToString:@"osirixplugin"])
			{
...
//				if (filterClass = [plugin principalClass])	
				{					
                    ...
                    //////////////
                    if ([[name pathExtension] isEqualToString:@"horlixplugin"])
                    {
                        [pluginDescription setObject:@"YES" forKey:@"HorosCompatiblePlugin"];
                    }
                    else
