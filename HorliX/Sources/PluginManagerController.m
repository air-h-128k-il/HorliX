...
- (IBAction) showWindow:(id)sender;//window response
{
...
#pragma mark Horos web page //pragma mark
...
- (void) setURLforHorosPluginWithName:(NSString*) name
{
    ...
    ////////////////////////////
    NSLog(@"before for loop ");
    
    for (NSDictionary *plugin in availablePlugins)
    {
        NSLog(@"entering for-loop");
        if([[plugin valueForKey:@"name"] isEqualToString:name])
        {
            ...
            NSLog(@"line 651");
            NSLog([plugin valueForKey:@"url"]);
            NSLog(name);
            NSLog([plugin valueForKey:@"name"]);
            BOOL alreadyInstalled = NO;
            BOOL sameName = NO;
            BOOL sameVersion = NO;
            for(NSDictionary *installedPlugin in plugins)
...
- (void)webView:(WebView *)sender decidePolicyForNavigationAction:(NSDictionary *)actionInformation request:(NSURLRequest *)request frame:(WebFrame *)frame decisionListener:(id<WebPolicyDecisionListener>)listener;
{
    ...
    {
        NSLog(@"not osirix and not horos");
        [listener use];
    }
