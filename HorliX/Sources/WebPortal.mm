...
{
    #ifdef MACAPPSTORE
	DefaultWebPortalDatabasePath = [[NSString alloc] initWithString: [@"~/Library/Application Support/HorliX App/WebUsers.sql" stringByExpandingTildeInPath]];
    #else
    DefaultWebPortalDatabasePath = [[NSString alloc] initWithString: [@"~/Library/Application Support/HorliX/WebUsers.sql" stringByExpandingTildeInPath]];
    #endif
...
+(void)observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context {
	if (!context) {
		...
		if ([keyPath isEqualToString:valuesKeyPath(OsirixWebPortalPrefersCustomWebPagesKey)])
        {
            ...
            #ifdef MACAPPSTORE
			if (NSUserDefaults.webPortalPrefersCustomWebPages) [dirsToScanForFiles addObject: [@"~/Library/Application Support/HorliX App/WebServicesHTML" stringByExpandingTildeInPath]];
            #else
            if (NSUserDefaults.webPortalPrefersCustomWebPages) [dirsToScanForFiles addObject: [@"~/Library/Application Support/HorliX/WebServicesHTML" stringByExpandingTildeInPath]];
            #endif
