...
// horos to horlix air
	id identity = (id)[DDKeychain KeychainAccessPreferredIdentityForName:@"info.horlix.horlixwebserver" keyUse:CSSM_KEYUSE_ANY];
	if (identity == nil)
	{
		[DDKeychain createNewIdentity];
		identity = (id)[DDKeychain KeychainAccessPreferredIdentityForName:@"info.horlix.horlixwebserver" keyUse:CSSM_KEYUSE_ANY];
	}
...
//    [response.httpHeaders setObject: @"no-cache" forKey: @"Cache-Control"];
	...
	if ([self.requestedPath hasPrefix:@"/weasis/"])
	{
		#ifndef OSIRIX_LIGHT
        BOOL assigned = false;
        for (NSString *dir in [HorliX WeasisCustomizationPaths]) {
...
-(void)replyToHTTPRequest {
	...
	if( webPortalDefaultTitle.length == 0)
		webPortalDefaultTitle = NSLocalizedString(@"HorliX Web Portal", @"Web Portal, general default title");
